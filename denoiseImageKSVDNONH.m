function [mseblk ksvd_step_output psnr_step D_step IOut Dictionary Coefs] = denoiseImageKSVDNONH(O_image,Image,matrix_sigma,bb,K,C,slidingDis,L)
%==========================================================================
[NN1,NN2]                    = size(Image);
...
% 4 对字典DCT进行训练
%   [ksvd_step_output psnr_step D_step Dictionary]              = KSVDNONH(blkMatrix,DCT,BETA,errorGoal,Image,BETA_sig,sigma,L);
%   DCT                       = Dictionary;
...
%5 用训练好的字典DCT进行去噪
  ...
  Coefs                     = OMPerrNONHtest(DCT,blocks,blocksbeta,errT,8);%%%%%%%%%求对应的系数
%   Coefs                     = OMPerrNONHtestnew(DCT,blocks,blocksbeta,errT,8);
  if (reduceDC)
